...
if (!CLIENT_ID || !CLIENT_SECRET) {
    console.error('❌ Error: COGGLE_CLIENT_ID and COGGLE_CLIENT_SECRET must be set in your .env or .dev.vars file.');
    console.error('Please get them from https://coggle.it/developer and add them to .dev.vars');
    process.exit(1);
}
...
app.get('/callback', async (req, res) => {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            return res.status(500).send(`Failed to get token: HTTP ${response.status} - ${errorText}`);
        }
        ...
        if (content.includes('COGGLE_API_TOKEN=')) {
            content = content.replace(/COGGLE_API_TOKEN=.*/g, `COGGLE_API_TOKEN=${data.access_token}`);
        } else {
    ...
    `);
        ...
        console.log('You can now press Ctrl+C to stop this server and run `npm run dev` to start the Cloudflare Worker.');
