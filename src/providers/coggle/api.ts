...
export class CoggleAPI {
    ...
    async listDiagrams(): Promise<CoggleApiDiagram[]> {
        return this.request('/api/1/diagrams', 'GET') as Promise<CoggleApiDiagram[]>;
    }

    async createDiagram(title: string): Promise<CoggleApiDiagram> {
        return this.request('/api/1/diagrams', 'POST', { title }) as Promise<CoggleApiDiagram>;
    }
    ...
    async addNode(diagramId: string, parent: string, text: string, offset?: { x: number, y: number }): Promise<any> {
    ...
    async updateNode(diagramId: string, nodeId: string, updates: { text?: string, parent?: string, offset?: { x: number, y: number } }): Promise<any> {
        return this.request(`/api/1/diagrams/${diagramId}/nodes/${nodeId}`, 'PUT', updates);
    }

    async deleteNode(diagramId: string, nodeId: string): Promise<any> {
        return this.request(`/api/1/diagrams/${diagramId}/nodes/${nodeId}`, 'DELETE');
    }
}
