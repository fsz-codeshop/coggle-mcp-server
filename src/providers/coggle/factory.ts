	...
	registerTools(server: McpServer, api: CoggleAPI): void {
		...
		server.tool(
			...
			"Retrieve the complete list of nodes and content structure of a specific diagram by its ID",
			{
				diagram_id: z
					.string()
					.describe("The unique identifier of the diagram to retrieve"),
			},
			async ({ diagram_id }) => {
		...
		server.tool(
			...
			"Add a new node to a diagram",
			{
				...
				parent: z
					.string()
					.describe("The ID of the parent node to attach the new node to"),
				text: z.string().describe("The text content of the new node"),
				offset_x: z
					.number()
					.optional()
					.describe(
						"Optional horizontal offset from the parent node (positive is right)",
					),
				offset_y: z
					.number()
					.optional()
					.describe(
						"Optional vertical offset from the parent node (positive is down)",
					),
			},
		...
		server.tool(
			...
			"Modify an existing node in a diagram",
			{
				...
				text: z
					.string()
					.optional()
					.describe("The new text content for the node (optional)"),
				parent: z
					.string()
					.optional()
					.describe("The ID of a new parent node (optional)"),
				offset_x: z
					.number()
					.optional()
					.describe("Optional new horizontal offset"),
				offset_y: z
					.number()
					.optional()
					.describe("Optional new vertical offset"),
			},
