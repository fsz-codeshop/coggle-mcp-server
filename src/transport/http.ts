...
export async function handleHttpMcp(
    ...
    if (body && typeof body === 'object' && 'method' in body) {
        console.log(`${reqId} MCP ${body.method} id=${(body as Record<string, any>).id || 'null'}`)

        // Sanitize body to avoid leaking sensitive prompt context
        const sanitizedBody = { ...body } as Record<string, any>
        if (sanitizedBody.params?.arguments) {
            sanitizedBody.params = { ...sanitizedBody.params, arguments: '[REDACTED]' }
        }

        console.log(`${reqId} REQUEST: ${JSON.stringify(sanitizedBody)}`)
